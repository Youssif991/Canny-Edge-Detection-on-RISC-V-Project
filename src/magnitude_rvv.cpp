 ...
 * @brief   Gradient magnitude — L1 (|Gx| + |Gy|), vectorized via RVV, templated on LMUL.
 *
 * The algorithm is written once in MagL1<LMUL>. rvv_traits<LMUL> abstracts
 * every intrinsic that changes with LMUL so the loop body is LMUL-agnostic.
 *
 * IMPORTANT: vredmax's destination register is ALWAYS LMUL=1 regardless of
 * the input LMUL. v_max_magnitude is intentionally vuint16m1_t — this is an
 * ISA rule, not a choice, and is therefore NOT part of the traits.
 */
...
namespace processing
{
    ...
    struct rvv_traits<1>
    {
        ...
        using u32 = vuint32m2_t; // widened: m1 * 2 = m2
        using u8 = vuint8mf2_t;  // narrowed: m1 / 2 = mf2
        ...
        // Reduction dest is always m1 — ISA rule, not parameterized
    ...
    struct rvv_traits<2>
    {
        ...
        using u32 = vuint32m4_t; // widened: m2 * 2 = m4
        using u8 = vuint8m1_t;   // narrowed: m2 / 2 = m1
    ...
    struct rvv_traits<4>
    {
        ...
        using u32 = vuint32m8_t; // widened: m4 * 2 = m8
        using u8 = vuint8m2_t;   // narrowed: m4 / 2 = m2
                 ...
                 const int16_t *__restrict Gy)
    {
        ...
        vuint16m1_t v_max_magnitude = __riscv_vmv_v_x_u16m1(0, 1); // reduction dest — always m1 (ISA rule)
        ...
        for (uint32_t i = 0; i < pixel_count; i += vl)
        {
            ...
            typename T::i16 v_gx = T::vle16_i(Gx + i, vl);
            typename T::i16 v_abs_gx = T::vmax_i(v_gx, T::vneg_i(v_gx, vl), vl); // abs = max(v, -v)
            typename T::u16 v_u_abs_gx = T::reinterpret(v_abs_gx);

            typename T::i16 v_gy = T::vle16_i(Gy + i, vl);
            typename T::i16 v_abs_gy = T::vmax_i(v_gy, T::vneg_i(v_gy, vl), vl);
            typename T::u16 v_u_abs_gy = T::reinterpret(v_abs_gy);

            typename T::u16 v_raw_mag = T::vadd_u(v_u_abs_gx, v_u_abs_gy, vl);
            T::vse16_u(magnitude_buffer.get() + i, v_raw_mag, vl);

            v_max_magnitude = T::vredmax(v_raw_mag, v_max_magnitude, vl); // dest always m1 — ISA rule
        }
        ...
        for (uint32_t i = 0; i < pixel_count; i += vl)
        {
            ...
            typename T::u16 v_mag = T::vle16_u(magnitude_buffer.get() + i, vl);
            typename T::u32 v_scaled = T::vwmulu_u(v_mag, 255, vl);     // x255 (widen u16->u32)
            v_scaled = T::vdivu_u(v_scaled, max_magnitude, vl);         // / max
            typename T::u16 v_narrow1 = T::vncvt_u32_u16(v_scaled, vl); // u32 -> u16
            typename T::u8 v_narrow2 = T::vncvt_u16_u8(v_narrow1, vl);  // u16 -> u8
    ...
    template Status MagL1<1>(const image::io::metadata_t<uint8_t> &,
                             const int16_t *__restrict,
                             const int16_t *__restrict);

    template Status MagL1<2>(const image::io::metadata_t<uint8_t> &,
                             const int16_t *__restrict,
                             const int16_t *__restrict);

    template Status MagL1<4>(const image::io::metadata_t<uint8_t> &,
                             const int16_t *__restrict,
                             const int16_t *__restrict);
